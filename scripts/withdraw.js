...
async function main() {
    ...
    for (let i = 2; i <= 5; i++) {
        ...
        if (result > 0) {
            const subcontract = new ethers.Contract(configs.contract_address, ABI.abi, wallet)
            const withdraw = await subcontract.withdrawFunds()
    ...
    if (result > 0) {
        const withdraw = await contract.withdrawFunds()
        console.log(withdraw)
    }
}
